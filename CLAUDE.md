...
Layout: routes in `src/routes/` (TanStack Router file-based routing), data layer in `src/db/` (Drizzle schema + D1 client) with migrations in `drizzle/`, server-side feature logic in `src/features/<name>/`, shared helpers in `src/utils/`.
